...
function Assert-MatchingFileContent {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SourcePath,
    ...
    Assert-RequiredPath -Path $DestinationPath -Description $Description -PathType Leaf
    $SourceHash = (Get-FileHash -LiteralPath $SourcePath -Algorithm SHA256).Hash
    $DestinationHash = (Get-FileHash -LiteralPath $DestinationPath -Algorithm SHA256).Hash
    if ($SourceHash -ne $DestinationHash) {
        throw "Packaged $Description does not match its current source: $DestinationPath"
    }
}

function Assert-MatchingFileTree {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SourceRoot,
    ...
    $SourceFiles = @(Get-ChildItem -LiteralPath $ResolvedSourceRoot -Filter $Filter -File -Recurse)
    $DestinationFiles = @(Get-ChildItem -LiteralPath $DestinationRoot -Filter $Filter -File -Recurse)
    if ($DestinationFiles.Count -ne $SourceFiles.Count) {
        throw "The packaged $Description file count does not match the current source ($($SourceFiles.Count))."
    }

    foreach ($SourceFile in $SourceFiles) {
        $RelativePath = $SourceFile.FullName.Substring($ResolvedSourceRoot.Length).TrimStart("\", "/")
        $DestinationPath = Join-Path $DestinationRoot $RelativePath
        Assert-MatchingFileContent -SourcePath $SourceFile.FullName -DestinationPath $DestinationPath -Description "$Description file $RelativePath"
    }
}
...
Assert-RequiredPath -Path $OptionalToolsInfo -Description "optional tools information file" -PathType Leaf
...
# Links to optional third-party clients; their executables are intentionally not bundled.
Copy-Item $OptionalToolsInfo (Join-Path $ReleaseFolder "Tools\DCS-gRPC\OPTIONAL-TOOLS.txt") -Force
...
$RequiredReleaseFiles = @(
    ...
    "Tools\DCS-gRPC\OPTIONAL-TOOLS.txt",
...
$PackagedServerDll = Join-Path $ReleaseFolder "Mods\tech\DCS-gRPC\dcs_grpc.dll"
$PackagedRepl = Join-Path $ReleaseFolder "Tools\DCS-gRPC\repl.exe"
$PackagedOptionalToolsInfo = Join-Path $ReleaseFolder "Tools\DCS-gRPC\OPTIONAL-TOOLS.txt"
$PackagedMission = Join-Path $ReleaseFolder "Missions\DCS-gRPC-Example.miz"
$ToolsProtosDir = Join-Path $ReleaseFolder "Tools\DCS-gRPC\protos\dcs"
Assert-MatchingFileContent -SourcePath $ServerDllPath -DestinationPath $PackagedServerDll -Description "server DLL"
Assert-MatchingFileContent -SourcePath $ReplExecutablePath -DestinationPath $PackagedRepl -Description "REPL executable"
Assert-MatchingFileContent -SourcePath $OptionalToolsInfo -DestinationPath $PackagedOptionalToolsInfo -Description "optional tools information file"
Assert-MatchingFileContent -SourcePath $SampleMission -DestinationPath $PackagedMission -Description "sample mission"
Assert-MatchingFileTree -SourceRoot $ProtosSourceDir -DestinationRoot $ToolsProtosDir -Filter "*.proto" -Description "Tools protobuf"
Assert-MatchingFileTree -SourceRoot $ProtosSourceDir -DestinationRoot $DocsProtosDir -Filter "*.proto" -Description "Docs protobuf"
