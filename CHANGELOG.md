...
- move to v0.10.0 after merge from Lenny's version branch

## [0.9.3] - 2026-09-12

### Changed

- `lua/DCS-gRPC/version.lua` is no longer tracked by git. It is generated by `build.rs` on every `cargo build` from the workspace version in `Cargo.toml`, and keeping it committed made every release branch switch leave a dirty copy behind that blocked `git pull` on `main`. `build.rs` now reads the version at run time and only rewrites the file when its content changes; `build_release.ps1` verifies the generated version matches `Cargo.toml` before packaging.
