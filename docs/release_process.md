...
`lua/DCS-gRPC/version.lua` is generated by `build.rs` from this version on every build and is ignored by git, so it never needs to be edited or committed. If it ever shows up as modified in `git status`, discard it with `git checkout -- lua/DCS-gRPC/version.lua`.
